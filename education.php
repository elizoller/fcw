...
$stmt1 = $db->prepare('SELECT college_name, college_dean, college_img FROM colleges WHERE colleges.college_id=3');
$stmt1->execute();
...
?>
	...
	<div id="rightnav">
	<ul id="nav">
	<div id="topTitle">Home</div><li class="current education_nav"><a href="#education" id="top"></a></li>
	</ul>
	<ul id="subnav">
	...
	<div id="kinesiologyTitle">Kinesiology</div><li class="education_nav"><a href="#9" id="kinesiology"></a></li>
	<!--this dynamically adds if there is a dean section-->
	<?php
	$stmt4 = $db->prepare("SELECT * FROM faculty, departments WHERE departments.department_id=faculty.department_id AND faculty.department_id=36 AND faculty.college_id=3");
	$stmt4->execute();
    ...
    <!-- Section #1 -->
	<section id="education" data-speed="4" data-type="background">
		<div class="row">
	...
	while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		if ($row['department_id'] != $deptid) {
				echo '<section id="' . $row['department_id'] . '" class="education_departments" data-speed="4" data-type="background"><div class="row main-education"><h1 id="educationtop">' . $college_name . '</h1><h3>Dean: ' . $college_dean . '</h3>';
	...
	$rowcount = $stmt4->rowCount();
	if ($rowcount > 0) {
		echo '<section id="36" class="education_departments" data-speed="4" data-type="background"><div class="row main-education"><h1 id="educationtop">' . $college_name . '</h1><h3>Dean: ' . $college_dean . '</h3>';
   ...
   <script src="deploy/scripts/jquery.min.js"></script>
   		...
   		$(document).ready(function() {   			
   			$("#subnav").onePageNav({
   				...
   				scrollChange: function($currentListItem) {
   					$('.education_departments').removeClass('curr');
   					//$('.education_nav').removeClass('current');
	...
	$("#arrow").on('click', function(e) {
   	...
   	<script>
   	$("#curriculum").on('mouseover', function() {
   		$("#curriculumTitle").fadeIn('slow','linear');
   	});
   	$("#curriculum").on('mouseout', function() {
   		$("#curriculumTitle").fadeOut('slow', 'linear');
   	});
   	$("#leadership").on('mouseover', function() {
   		$("#leadershipTitle").fadeIn('slow','linear');
   	});
   	$("#leadership").on('mouseout', function() {
   		$("#leadershipTitle").fadeOut('slow', 'linear');
   	});
   	$("#kinesiology").on('mouseover', function() {
   		$("#kinesiologyTitle").fadeIn('slow','linear');
   	});
   	$("#kinesiology").on('mouseout', function() {
   		$("#kinesiologyTitle").fadeOut('slow', 'linear');
   	});
